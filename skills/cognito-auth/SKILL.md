...
The pool's Terraform takes two variables, `cognito_email_from` and `cognito_email_ses_identity_arn`. Both empty (the default) leaves the pool on Cognito's default sender, which AWS caps at **50 emails a day per AWS account** — an allowance shared by every app in the account, and one that three apps' verifiers emptied between them on 2026-09-10. Both set switches the pool to `DEVELOPER` sending through that SES identity. **A production app must set both**; the default sender is for a build's smoke test only. The identity is for **account, two-factor authentication, and account-recovery email only — no marketing**, and the delivered app's own documentation must say so, because that is the basis on which the account's SES production access was requested.
...
| --- | --- |
| `references/terraform.md` | Provisioning the user pool + public app client, the `USER_PASSWORD_AUTH` flow, verification-required in every environment (and why there is no dev bypass), the `cognito_email_from` / `cognito_email_ses_identity_arn` variables that switch the pool from Cognito's 50-a-day default sender to the account's SES identity, and the outputs the app needs. |
...
- **Set the two email variables for any app real visitors will use.** `cognito_email_from` and `cognito_email_ses_identity_arn` move the pool off the shared 50-a-day default sender. Declare them with the exact names in `references/terraform.md`, and record in the app's docs that the identity sends account, 2FA and recovery mail only.
